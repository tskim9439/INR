...
class AudioFile1(torch.utils.data.Dataset):
    def __init__(self, filename):
        self.rate, self.data = wavfile.read(filename)
        print(self.rate)
        self.data = self.data.astype(np.float32)[:len(self.data)//4]
        self.timepoints = get_mgrid(len(self.data), 1)
        self.timepoints = torch.cat([self.timepoints,
                                     torch.tensor([np.sin(i * math.pi * 10) for i in range(len(self.data))], dtype=torch.float).unsqueeze(-1)],
                                    dim=-1)
        print(self.timepoints.shape)
